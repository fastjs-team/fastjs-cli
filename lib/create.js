...
const isDev = pathExistsSync(join(process.cwd(), "ondev.key"));
...
async function create(name, options) {
  ...
  const npmVersion = execSync("npm -v", {stdio: "pipe"}).toString()
  ...
  try {
    await execSync(`npm install fastjs-next fastjs-cli`, {cwd: createPath, stdio: isDev && 'inherit' || 'ignore'})
  } catch (error) {
    await outputDev(error);
    err(
      "Error 940: Error when doing command",
  ...
  if (autoInstall) {
    ...
    try {
      await execSync(`npm install`, {cwd: createPath, stdio: isDev && 'inherit' || 'ignore'})
    } catch (error) {
  ...
  if (cssLoader !== "css") {
    ...
    try {
      await exec(`npm install ${cssLoader} ${cssLoader}-loader`, {cwd: createPath, stdio: isDev && 'inherit' || 'ignore'})
    } catch (error) {
      // print log (<run-path>/fastjs-cli-<timestamp>.error.log)
      const errorLog = path.join(runPath, `fastjs-cli-${startTime}.error.log`);
      fs.createFileSync(errorLog);
  ...
  modules.forEach((item) => {
    ...
    moduleCliConfig.depends.forEach(async (depend) => {
      // child process disallow output to console
      load.update(`Install ${item} depend ${depend}`, "+1", "+1");
      outputDev(`Install ${item} depend ${depend}`);
      await execSync(`npm install ${depend}`, {cwd: createPath, stdio: isDev && 'inherit' || 'ignore'});
    })
    // mkdir
