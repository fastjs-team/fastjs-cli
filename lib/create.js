...
async function create(name, options) {
  ...
  modules.forEach((item) => {
    ...
    if (!itemSuper.parent.includes(projectTypeSuper)) {
...
function nextToUpdatePackage() {
  ...
  viteConfig.plugins = []
  // if vue
  if (projectTypeSuper.includes("vue")) {
    outputDev("Add vue plugin");
    viteConfig.plugins.push("vue()");
    viteImport.push("import vue from '@vitejs/plugin-vue'");
  }
