  ...
  exec(`npm create vite@latest fastjs_cli_create ${EXTRA_ARGS}--template ${projectTypeSuper}`, {
    ...
    exec(`npm install fastjs-next`, {cwd: createPath, stdio: 'ignore'}, (error) => {
      ...
      if (installAfterSetup) {
        ...
        try {
          execSync(`npm install --local`, {cwd: createPath, stdio: 'ignore'})
        } catch (e) {
      ...
      if (cssLoader !== "css") {
        ...
        execSync(`npm install ${cssLoader} ${cssLoader}-loader --local`, {cwd: createPath, stdio: 'ignore'});
      }
      // if vue
      ...
      // install modules
      modules.forEach((module) => {
        spinner.text = `Install module ${module}`
